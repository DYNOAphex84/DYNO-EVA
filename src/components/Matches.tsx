...
import { Plus, Calendar, Clock, MapPin, X, Check, Play, Trophy, Download } from 'lucide-react';
...
export default function Matches({ matches, players, onAddMatch }: MatchesProps) {
  ...
  const [newMatch, setNewMatch] = useState<Partial<Match>>({
    date: '', time1: '', time2: '', arena: 'Arena 1',
    opponent: '', division: 'Division 1', status: 'upcoming', lineup: [], scores: [],
  });

  const filtered = filter === 'all' ? matches : matches.filter((m) => m.status === filter);
  const sorted = [...filtered].sort((a, b) =>
    new Date(`${b.date}T${b.time1}`).getTime() - new Date(`${a.date}T${a.time1}`).getTime()
  );

  const handleSubmit = () => {
    if (!newMatch.date || !newMatch.opponent || !newMatch.time1) return;
    const match: Match = {
      id: `m${Date.now()}`, date: newMatch.date!, time1: newMatch.time1!,
      time2: newMatch.time2 || undefined, arena: (newMatch.arena as 'Arena 1' | 'Arena 2') || 'Arena 1',
      opponent: newMatch.opponent!, division: newMatch.division || 'Division 1',
      status: 'upcoming', lineup: newMatch.lineup || [], scores: [],
    };
    onAddMatch(match);
    setShowForm(false);
    setNewMatch({ date: '', time1: '', time2: '', arena: 'Arena 1', opponent: '', division: 'Division 1', status: 'upcoming', lineup: [], scores: [] });
  };
  ...
  const addToCalendar = (match: Match) => {
    ...
    const title = encodeURIComponent(`EVA vs ${match.opponent}`);
    const details = encodeURIComponent(`Match EVA Esport Arena\nArène: ${match.arena}\nDivision: ${match.division}`);
    const icsUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${title}&dates=${dateStr}T${timeStr}/${dateStr}T${timeStr}&details=${details}`;
        ...
        {(['all', 'upcoming', 'live', 'completed'] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-2 rounded-lg text-xs font-rajdhani font-semibold border transition-all ${
              filter === f ? 'bg-neon-blue/10 border-neon-blue/30 text-neon-blue' : 'bg-dark-700 border-dark-400/30 text-gray-400 hover:text-white'
            }`}>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="font-orbitron text-lg font-bold text-white">{match.opponent}</h3>
                      <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] ${statusConfig.bg} ${statusConfig.color} ${statusConfig.border} border`}>
                        {statusConfig.icon} {statusConfig.label}
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-3 text-xs text-gray-400">
                      <span className="flex items-center gap-1"><Calendar size={12} /> {match.date}</span>
                      <span className="flex items-center gap-1"><Clock size={12} /> {match.time1}{match.time2 && ` / ${match.time2}`}</span>
                      <span className="flex items-center gap-1"><MapPin size={12} /> {match.arena}</span>
                  ...
                  {result && (
                    <div className="flex items-center gap-2">
                      <span className={`font-orbitron text-2xl font-bold ${result.won ? 'text-neon-green' : 'text-neon-red'}`}>
                        {result.teamWins} - {result.oppWins}
                      </span>
                    </div>
                  )}
                  ...
                  {match.status === 'upcoming' && (
                    <button onClick={() => addToCalendar(match)}
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-neon-purple/10 border border-neon-purple/30 text-neon-purple text-xs hover:bg-neon-purple/20 transition-all">
                      <Download size={12} /> Calendrier
                    </button>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70">
          <div className="glass-panel rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-orbitron text-lg font-bold text-white">Nouveau match</h2>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-gray-400 mb-1 block font-rajdhani">Heure 1</label>
                  <input type="time" value={newMatch.time1} onChange={(e) => setNewMatch({ ...newMatch, time1: e.target.value })} className="w-full text-sm" />
                </div>
                <div>
                  <label className="text-xs text-gray-400 mb-1 block font-rajdhani">Heure 2 (optionnel)</label>
                  <input type="time" value={newMatch.time2} onChange={(e) => setNewMatch({ ...newMatch, time2: e.target.value })} className="w-full text-sm" />
                </div>
              </div>
              <div>
                <label className="text-xs text-gray-400 mb-1 block font-rajdhani">Arène</label>
                <select value={newMatch.arena} onChange={(e) => setNewMatch({ ...newMatch, arena: e.target.value as 'Arena 1' | 'Arena 2' })} className="w-full text-sm">
